 <?php
  include './aside-left.component.html/';

  $AdresseMail = "bloullittaa@gmail.com";
  # si le bouton Envoyer est cliqué
  if (isset($_POST['envoyer'])) { //je vérifie que le champ mail est correctement rempli
    if (empty($_POST['mail'])) {
      echo "Le champ mail est vide";
    } else { //j vérifie que l'adresse est correcte avec les exigences
      if (!preg_match("#^[a-z0-9_-]+((\.[a-z0-9_-]+){1,})?@[a-z0-9_-]+((\.[a-z0-9_-]+){1,})?\.[a-z]{2,}$#i",
       $_POST['mail'])) {
        echo "L'adresse mail entrée est incorrecte";
      } else { //on vérifie que le champ sujet est correctement rempli
        if (empty($_POST['sujet'])) {
          ...
          if (empty($_POST['message'])) {
            ...
            //tout est correctement renseigné, on envoi le mail //on renseigne les entêtes de la fonction mail de PHP
